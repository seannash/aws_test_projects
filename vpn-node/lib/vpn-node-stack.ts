...
export interface VpnNodeStackProps extends cdk.StackProps {
  /**
...
export class VpnNodeStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: VpnNodeStackProps) {
    super(scope, id, props);

    const keyPairName = props?.keyPairName || 'vpn-node-keypair';
    ...
    const instance = new lightsail.CfnInstance(this, 'TorNodeInstance', {
      instanceName: 'vpn-node-instance-v2',
      ...
      // - Tor OR port (9013): For relay connections from other Tor nodes
      // - WireGuard port (51820): For VPN connections
      networking: {
        ports: [
          {
